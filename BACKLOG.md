...
### B02 — 记忆同步破坏性写入
**问题**：`/api/memory/sync` 先 DELETE 再 INSERT，中途失败会导致该 session 记忆永久丢失，无报错、无回滚。
**方案**：改为 upsert，或加事务保护（Supabase RPC + BEGIN/COMMIT）。
...
### B09 — 429 前端无友好提示
**问题**：限流触发后前端收到 429 纯文本，`useChatStream.ts` 未处理，用户看不到原因。
**方案**：在 SSE 消费逻辑中拦截 429，显示"请求过于频繁，请稍后再试"。
...
### B11 — `user_vectors` 旧表残留
代码已切换到 `memories_tier1` 架构，但旧表和 RLS 策略仍在 Supabase，是技术债。
...
### B15 — 移除废弃依赖
`ai`（Vercel AI SDK）和 `@ai-sdk/google` 仍在 `package.json`，实际已不使用。
