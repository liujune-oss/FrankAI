...
## v1.8.89 ~ v1.8.99 — 2026-03-08

### 语音体验全面升级
- **v1.8.87**：主聊天界面新增麦克风按钮，录音→云端 Gemini STT→文字回填输入框等待发送
- **v1.8.89**：语音转写和记忆摘要提示语统一中文，系统指令禁止中文字间加空格
- **v1.8.91**：活动列表卡片（Tasks/Calendar）新增语音备注按钮（原删除按钮位），删除操作移至详情页
- **v1.8.92**：STT 提示语明确禁止推测性输出（"好的"等），前端增加 blob < 4KB 静音过滤
- **v1.8.93**：活动详情页 header 新增语音备注按钮
- **v1.8.98**：日历页新增全局语音 FAB，支持语音创建任意类型活动

### 活动管理 UI 重构
- **v1.8.90**：同会话记忆摘要改为覆盖写入（delete+insert），记忆删除改为乐观更新
- **v1.8.94**：过期未完成活动显示红色警告背景 + "已过期"角标，强制用户处理
- **v1.8.95**：任务列表已完成条目默认折叠，"已完成 (N) ▶" 点击展开
- **v1.8.96**：已完成折叠区仅显示最近 15 天内完成的条目（按 updated_at 过滤）
- **v1.8.97**：任务标签从单选改为多选，新增里程碑（amber）和提醒（pink）标签，过滤偏好 localStorage 持久化
- **v1.8.98**：日历页卡片新增完成状态切换（CheckSquare/Square）
- **v1.8.99**：抽屉导航"待办"→"活动"/"日程"→"日历"；日历显示所有带时间活动（含里程碑），补全 milestone 颜色点和卡片样式

### 记忆系统优化
- **v1.8.88**：记忆管理 UI 全面中文化，错误提示、按钮文案统一简体中文
- **v1.8.90**：记忆摘要生成提示语改为中文，对话标签"User/Assistant"→"用户/助手"，同会话摘要改为单条覆盖
