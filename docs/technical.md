...
> 最后更新：2026-03-05（与 v1.8.42 代码同步）
...
> `ai`（Vercel AI SDK）和 `@ai-sdk/google` 已于 v1.8.37 从 package.json 移除。
> 聊天流完全基于 `@google/generative-ai` 原生 SDK 和手写 SSE 实现。
...
**关键设计决策**：
- **隔离确认模型（v1.8.35）**：工具执行完成后，用一个全新的、无对话历史的模型来生成确认文字，彻底防止模型混淆历史上下文。
- **Phase 2 三层兜底（v1.8.42）**：确认模型的响应保证不为空：① 流式文字；② `confirmResult.response` fallback；③ 硬编码"xxx 已创建成功"。
- **调试面板仅开发环境显示（v1.8.40）**：`process.env.NODE_ENV === 'development'` 控制，生产环境不暴露。
- `maxDuration = 120`（秒）适配 Pro 模型的思考时间。
- `@ts-nocheck` 目前加在文件顶部，待后续补充类型（BACKLOG #B03）。
...
### 4. 三层记忆 RAG 系统（v1.8.38+）

> 完整设计见 `docs/memory-architecture-v2.md`

#### 架构概览

```
System Prompt 注入（每次 chat）
├── 热层 Core Memory    user_core_memory 表，常驻，~150 token
├── 温层 Recall Memory  memories_chunks 最近 5 条，按时间取，~400 token
└── 冷层 Archival       向量搜索 top 3（排除温层），~300 token
```

#### 写入（`/api/memory/sync`）

触发条件（`useChatStream.ts`）：
- **阈值触发**：未同步消息 ≥ 10 条（1 个 chunk）
- **空闲触发**：聊天结束后 1 分钟无新消息
- **页面关闭兜底**：`visibilitychange` 事件 + Beacon API，防止关页丢失

写入流程（**增量追加，不删除旧数据**）：
```
POST /api/memory/sync { session_id, chunk_index, messages[] }
  → 写入 chat_messages（仅作日志，不用于 RAG）
  → gemini-flash 生成本块摘要（summary_text）
  → gemini-embedding-001 向量化
  → INSERT memories_chunks（追加，chunk_index 递增）
  → 异步：LLM 判断是否需要更新 user_core_memory
```
...
```
并行执行：
  ① SELECT user_core_memory WHERE user_id = ?          → 热层
  ② SELECT memories_chunks ORDER BY created_at DESC LIMIT 5  → 温层
  ③ embedContent(用户消息) → match_archival_memories RPC      → 冷层
...
| 表 | 用途 |
|---|---|
| `user_core_memory` | 每用户一行，长期稳定事实，LLM 异步维护 |
| `memories_chunks` | 按块存储摘要+向量，追加写入 |
| `chat_messages` | 原始消息日志，仅供管理后台查看 |

**索引策略**：pgvector ivfflat/hnsw 最高支持 2000 维，`gemini-embedding-001` 为 3072 维超出限制，故不建向量索引，使用**全表精确扫描（Exact KNN）**。个人用户数据量有限，性能无影响。
...
| `memory_chunk_size` | 10 | 客户端触发阈值（消息数） |
...
### 6. 会话管理（`lib/conversations.ts` + 云端同步）
...
Supabase conversations 表：
  id TEXT PK, user_id UUID, title TEXT, messages JSONB, created_at BIGINT, updated_at BIGINT

API 端点：/api/conversations/sync
  GET    → 拉取该用户全部对话
  POST   → upsert 单条对话（body: { conversation }）
  DELETE → 删除单条（body: { id }）或全部（body: { all: true }）
```

**双向合并策略**（`useConversations` 初始化时）：
```
Phase 1（同步）：从 IndexedDB 加载 → 立即显示 UI
Phase 2（后台）：GET 云端 → 与本地合并
  - 云端版本更新（updatedAt 较大）→ 写入 IndexedDB
  - 本地独有 → fire-and-forget POST 到云端
```
...
|------|------|
| 初始化 | 后台拉取并合并（不阻塞） |
| saveMessages | fire-and-forget upsert |
| 删除对话 | fire-and-forget DELETE |
| 清空全部 | fire-and-forget DELETE all |

**注意**：`images` 字段（base64 图片数据）**不同步到云端**，节省存储。跨设备后图片不可见，文字内容完整保留。
...
| `memory_summary_model` | gemini-2.0-flash | 记忆摘要用模型 |
| `memory_embedding_model` | gemini-embedding-001 | 向量嵌入模型（3072维） |
| `image_gen_model` | gemini-2.5-flash-image | 图片生成模型 |
| `memory_recall_count` | 5 | 温层取最近几条摘要 |
| `memory_archival_count` | 3 | 冷层向量搜索 top N |
...
-- 记忆系统（v1.8.38 架构 v2）
user_core_memory        user_id(PK), content(TEXT), updated_at          ← 热层，每人一行
memories_chunks         id, user_id, session_id, chunk_index, summary_text,
                        embedding(vector 3072), message_count, created_at  ← 温/冷层
chat_messages           id, user_id, session_id, role, content, created_at  ← 日志，不用于RAG

-- 旧表（已废弃，未删除）
memories_tier1          id, user_id, session_id, summary_text, embedding(vector),
                        start_message_id, end_message_id
user_vectors            id, user_id, content, embedding(vector 3072), metadata

-- 对话历史同步（v1.8.52）
conversations           id(TEXT PK), user_id UUID, title TEXT, messages JSONB,
...
├── scripts/
│   └── test-chat-flow.mjs                 # 工具调用循环本地单元测试（7场景，无网络）
├── build-sw.js                            # SW 构建脚本（Serwist）
├── version.json                           # 版本号 { major, minor, build }
├── supabase/
│   ├── init_app_config.sql                # app_config 初始化
│   ├── memory_v2_migration.sql            # 记忆架构 v2 迁移（v1.8.38）
│   └── calendar_tasks_schema.sql          # activities 表结构
└── package.json
```
